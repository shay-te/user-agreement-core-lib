...
class AgreementDocumentService(Service):
    ...
    def delete_by_user_ids(self, user_ids: list):
        return self._user_agreement_document_da.delete_by_user_ids(user_ids)

    def delete_document(self, name: str, language: str):
        time_now = datetime.utcnow()
        int_time_now = int(time_now.timestamp())
        document = self.get_document_latest_version(name, language)
        self._agreement_document_da.update(
            document[AgreementDocument.id.key],
            {
                AgreementDocument.name.key: f'{document[AgreementDocument.name.key]}_{int_time_now}',
                AgreementDocument.deleted_at.key: time_now,
            }
        )
